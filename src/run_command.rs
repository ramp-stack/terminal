...
use crate::line::Line;
use crate::state::spawn_streaming;

pub fn run_command(raw_cmd: &str, t: &Terminal) {
    ...
    if let Some(rest) = cmd.strip_prefix("cd") {
        ...
        match std::fs::canonicalize(&target) {
            Ok(p) if p.is_dir() => {
                t.set_cwd(p.to_string_lossy().into_owned());
            }
