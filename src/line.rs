use quartz::Color;
use crate::ansi::{StyledSpan, parse_ansi};
...
impl Line {
    pub fn output(s: impl Into<String>) -> Self {
        Self { text: s.into(), kind: LineKind::Output, spans_cache: None }
    }
    pub fn stderr(s: impl Into<String>) -> Self {
        Self { text: s.into(), kind: LineKind::Stderr, spans_cache: None }
    }
    pub fn error(s: impl Into<String>) -> Self {
        Self { text: s.into(), kind: LineKind::Err, spans_cache: None }
    }
    pub(crate) fn echo(s: impl Into<String>) -> Self {
        Self { text: s.into(), kind: LineKind::Echo, spans_cache: None }
    }
    pub fn precolored(segments: &[(&str, Option<Color>)]) -> Self {
        let spans = segments.iter().map(|(t, c)| StyledSpan {
            text: t.to_string(), color: *c,
        }).collect();
