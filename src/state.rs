...
impl State {
    ...
    pub fn content_h(&self, lh: f32) -> f32 { (self.buf.len() + 1) as f32 * lh }
...
pub(crate) fn spawn_streaming(shell: &str, cmd: &str, working_dir: &str) -> Result<(Receiver<StreamMsg>, Child), String> {
    ...
    std::thread::spawn(move || { let _ = t1.join(); let _ = t2.join(); let _ = tx.send(StreamMsg::Done(None)); });
    Ok((rx, child))
}
